...
try:
    ...
    print(
        "Warning: Error encountered parallelization of boot.generate() -- ignore if only using one core"
    )

# without parallelization
try:
    out = boot.generate(
        S_X, Y=S_y, method="tsne", B=4, save=False, random_seed=452, random_state=452
    )
except Exception:
...
assert round(out["x1"].values[333], 4) == round(
    truth["x1"].values[333], 4
), "Output dataframe does not match truth.csv"
assert round(out["x2"].values[1111], 4) == round(
    truth["x2"].values[1111], 4
), "Output dataframe does not match truth.csv"
...
assert round(variance_scores[333], 4) == round(
    truth[333], 4
), "Variance scores do not match truth_variances.txt"
...
for method in ["spearman", "distortion", "jaccard", "mean_projection_error", "stretch"]:
    try:
        concordance_scores = score.concordance(
            out, S_X, method=method, k=50, bootstrap_number=-1
        )
    except Exception:
        print(
            "Warning: Error encountered for score.concordance(method='" + method + "')"
        )
