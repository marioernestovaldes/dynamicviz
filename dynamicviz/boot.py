...
def dimensionality_reduction(X, method, **kwargs):
    ...
    if isinstance(method, str):
        if method == "tsne":
            ...
            X_embedded = skm.LocallyLinearEmbedding(
                n_components=2, **kwargs
            ).fit_transform(X)
        elif method == "mlle":
            X_embedded = skm.LocallyLinearEmbedding(
                n_components=2, method="modified", **kwargs
            ).fit_transform(X)
        elif method == "isomap":
            ...
            raise Exception(
                method + " is not a recognized dimensionality reduction algorithm"
            )
...
def bootstrap(
    ...
    **kwargs,
    ...
    if isinstance(num_jobs, int):  # in parallel
        ...
        for b in tqdm(range(B)):
            X_embedded, boot_idxs = run_one_bootstrap(
                X,
                method,
                sigma_noise,
                no_bootstrap,
...
def run_one_bootstrap(
    ...
    **kwargs,
    ...
    if subsample is False:
        if (
            no_bootstrap is True
        ):  # don't bootstrap (will use intrinsic stochasticity of DR algorithm (if any) only)
            boot_X = X.copy()
            boot_idxs = np.arange(X.shape[0])  # set indices to be the original indices
        elif (
            random_seeded_sequence is not False
        ):  # use specified random_seeded_sequence to generate bootstrap X
    ...
    # Subsample instead of bootstrapping
        ...
        if (
            random_seeded_sequence is not False
        ):  # use specified random_seeded_sequence to generate subsample of X
            seeded_rand2 = np.random.RandomState(random_seeded_sequence[b])
...
def generate(
    ...
    **kwargs,
    ...
    original_embedding = dimensionality_reduction(
        X, method, **kwargs
    )  # Ex: can specify random_state in **kwargs to remove stochasticity

    # reference points is the original dataset
    points0 = np.hstack(
        (
            original_embedding,
            np.zeros(original_embedding.shape[0]).reshape(
                original_embedding.shape[0], 1
            ),
        )
    )  # append uniform 3rd dimension

    # add basic info
    output["x1"] = points0[:, 0] - np.mean(
        points0[:, 0]
    )  # center reference visualization at (0,0)
    ...
    if B > 0:
        bootstrap_embedding_list, bootstrap_indices_list = bootstrap(
            X,
            method,
            B,
            sigma_noise,
    ...
    for i in range(len(bootstrap_embedding_list)):
        ...
        points = np.hstack(
            (points, np.zeros(points.shape[0]).reshape(points.shape[0], 1))
        )  # append uniform 3rd dimension
