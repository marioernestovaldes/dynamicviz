...
def interactive(
    ...
    if isinstance(colors, list):
        if len(colors) < len(np.unique(df[label])):
            raise Exception(
                "colors need to have >= length as len(np.unique(df[label]))"
            )
    ...
    for boot_num in np.sort(np.unique(df["bootstrap_number"])):
        ...
        # otherwise, discrete labels
            ...
            for li, lab in enumerate(np.unique(df[label])):
                if colors is None:
                    plt.scatter(
                        sub_df[sub_df[label] == lab]["x1"],
                        sub_df[sub_df[label] == lab]["x2"],
                        label=str(lab),
                        marker=marker,
                        alpha=alpha,
                        **kwargs,
            ...
            if show_legend is True:
                leg = plt.legend(
                    loc="center left", bbox_to_anchor=(1, 0.5), fontsize=legend_fontsize
                )
        ...
        if isinstance(get_frames, list):
            ...
            if boot_num in get_frames:
                plt.savefig(
                    os.path.join(save.split(".")[0], "frame" + str(boot_num) + ".pdf"),
                    dpi=dpi,
                )
    ...
    if isinstance(save, str):
        ...
        if ".gif" in save:
            images[0].save(
                ...
                optimize=False,
                duration=[1000] + [100] * len(images[1:]),
                loop=0,
            )
        # save as AVI
        elif ".avi" in save:
